# Daily Coach Report

- Generated: 2026-05-25T01:39:48.001534+10:00
- Conclusion: 恢复一般，今天更适合轻松有氧或降强度。
- Today suggestion: 避免双强度，同一天只保留一个主要刺激；更稳的是轻松跑、上肢力量或低负荷日。

## Data Status
- today_date: 2026-05-25
- recovery_date: 2026-05-24
- training_state_date: 2026-05-25
- recovery_is_fresh: False
- training_state_is_fresh: True

## Recovery
- date: 2026-05-24 00:00:00
- weight: 58.0
- resting_hr: 107.0
- hrv: None
- hrv_sdnn: None
- readiness: None
- sleep_secs: None
- sleep_hours: None
- sleep_score: None
- sleep_quality: None
- avg_sleeping_hr: None
- spo2: None
- fatigue: None
- soreness: None
- stress: None
- mood: None
- motivation: None
- injury: None

## 7-Day Recovery Trend
- window_start: 2026-05-19
- window_end: 2026-05-25
- days_with_recovery_data: 6
- avg_sleep_hours: 7.71
- min_sleep_hours: 6.28
- avg_sleep_score: 84.5
- avg_hrv: 65.0
- avg_resting_hr: 57.67
- avg_sleeping_hr: None
- low_sleep_score_days: 0

## Training State
- date: 2026-05-25 00:00:00
- ctl: 20.026083
- atl: 22.480371
- ramp_rate: -3.0076847
- ctl_load: 0.0
- atl_load: 0.0
- ride_eftp: None
- run_eftp: None
- atl_ctl_ratio: 1.12

## Yesterday Primary Activity
- id: i150952658
- start_date_local: 2026-05-24 12:09:23
- name: Canberra Running
- type: Run
- moving_time_sec: 1323
- elapsed_time_sec: 1328
- distance_m: 4005.23
- distance_km: 4.01
- average_heartrate: 156.0
- max_heartrate: 175.0
- average_speed: 3.023
- max_speed: 4.068
- total_elevation_gain: 29.90741
- calories: 269.0
- average_cadence: 83.15909
- pace: 3.0273848
- threshold_pace: 3.8022814
- icu_training_load: 23.0
- icu_intensity: 78.99138
- icu_fitness: 20.508615
- icu_fatigue: 25.93257
- icu_eftp: None
- icu_average_watts: None
- icu_normalized_watts: None
- selection_meta: {'selection_mode': 'primary_activity_of_day', 'candidate_count': 1, 'candidate_types': ['Run'], 'selected_type': 'Run', 'selected_training_load': 23.0, 'selected_moving_time_sec': 1323.0, 'selection_rule': '先选主训练类型（Run/Workout/Ride），再优先 Run，之后按训练负荷、时长、开始时间排序。', 'target_date': '2026-05-24'}
- classification: {'label': 'easy_aerobic_run', 'speed_vs_threshold_ratio': 0.795, 'reason': '整体更像常规有氧，而不是恢复跑；距离虽短但平均心率不低', 'distance_km': 4.01, 'duration_min': 22.1}

## 7-Day Activity Trend
- window_start: 2026-05-19
- window_end: 2026-05-25
- activity_count: 1
- total_training_load: 23.0
- avg_training_load: 23.0
- run_count: 1
- run_distance_km: 4.01

## Signals To Watch
- 今天晨间恢复数据可能还未同步完成，当前恢复判断可能仍沿用前一日数据
- 静息心率高于近7天均值，需留意疲劳或压力
